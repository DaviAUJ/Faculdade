...
Essa é minha implementação do MIPS ciclo único. Eu fiz o básico, depois adicionei o jump, tudo conforme
o livro que serviu de base. Esta é a segunda vez que eu envio, você disse na aula do dia 11/02 que 
...
e para executa-lo basta rodar o makefile. A implementação também é acompanhada de um programa usando quase 
...
é só mudar nas variáveis, não se preocupe porque eu já vou enviar no jeito para linux. Segunda mudança,
reestruturei os arquivos para eles não ficarem tudo numa pasta só, deixei mais organizado. Terceira mudança, 
...
Vou explicar como eu adicionei as instruções novas, os componentes adicionais estão todos em suas
...
então tive que fazer um sinal específico sair da ALUControl, o jrEnable, que vai fazer o controle dessa instrução.
Por ela ser R-Type, ela vai tentar escrever em algum registrador, sendo esse o $zero, pois no seu binário,
ela tem 00000 no registrador alvo, porém como existe uma restrição no banco de registradores que impede
esse em específico de ser alterado, então tudo flui bem. Tive que adicionar mais um multiplexador, antes do
...
um caso no case do ALUControl. Eu precisava dela para fazer um NOT bitwise nos registradores.